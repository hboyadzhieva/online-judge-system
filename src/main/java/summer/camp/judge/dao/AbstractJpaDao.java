 ...
 */
@Singleton
public abstract class AbstractJpaDao<Key, Entity extends IJPAEntity<Key>> {
	...
	/**
	 * Batch upsert a list of entities into the database
	 *
	 * @param entities
	 */
	public void bulkUpsert(final List<Entity> entities) {
		getEntityManager().getTransaction().begin();
		for (Entity e : entities) {
			if (findById(e.getKeyValue()) != null) {
				getLogger().debug(DEBUG_UPDATE_MESSAGE, e);
				getEntityManager().merge(e);
			} else {
				getLogger().debug(DEBUG_CREATE_MESSAGE, e);
